...
(() => {
    ...
    window.fetch = (input, init = {}) => {
        ...
        if (url.startsWith(API_BASE)) {
            ...
            const authToken = localStorage.getItem("finplan-token");
    ...
    function mount() {
        ...
        $("#authForm").addEventListener("submit", submit); $("#authSwitch").addEventListener("click", () => { updateMode(!registerMode); $("#authError").textContent = ""; }); updateMode(false);
    }
    ...
    function enterApp(user) { $("#authScreen").hidden=true; $(".app-shell").hidden=false; const name=user?.name||"Moheb"; if($(".profile-info strong"))$(".profile-info strong").textContent=name; if($("#welcomeName"))$("#welcomeName").textContent=name; window.dispatchEvent(new CustomEvent("finplan:authenticated",{detail:user})); }
    ...
    window.finplanLogout=()=>{localStorage.removeItem("finplan-token");localStorage.removeItem("finplan-user");location.href="/";};
    window.addEventListener("finplan:language-change",()=>{if($("#authScreen")&&!$("#authScreen").hidden)updateMode(registerMode);});
    document.addEventListener("DOMContentLoaded",()=>{$("#logoutButton")?.addEventListener("click",window.finplanLogout);init();});
})();