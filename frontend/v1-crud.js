...
(() => {
    ...
    async function submit(form) {
        ...
        try {
            const response = await fetch(`${API}/${kind === "transaction" ? "transactions" : "budgets"}`, { method:"POST", headers:{"Content-Type":"application/json","Authorization":`Bearer ${token()}`}, body:JSON.stringify(payload) });
            const body = await response.json().catch(() => ({}));
            if (response.status === 401) { window.dispatchEvent(new CustomEvent("finplan:auth-required")); return; }
            if (!response.ok) { error.textContent = response.status === 400 ? text("This category already has a budget for this month.","برای این دسته‌بندی در این ماه قبلاً بودجه ثبت شده است.") : text("Could not save. Please try again.","ذخیره انجام نشد. دوباره تلاش کن."); console.error("FinPlan CRUD",response.status,body); return; }
            $("#v1CrudModal")?.remove();
            toast(kind === "transaction" ? text("Transaction added successfully.","تراکنش با موفقیت ثبت شد.") : text("Budget added successfully.","بودجه با موفقیت ثبت شد."));
            await window.refreshDashboard?.();
            if (kind === "transaction") window.dispatchEvent(new CustomEvent("finplan:transactions-refresh"));
            else window.dispatchEvent(new CustomEvent("finplan:workspace", { detail:"budgets" }));
        } catch (e) { console.error(e); error.textContent = text("Could not connect to the server.","اتصال به سرور انجام نشد."); }
    }

    document.addEventListener("click", event => {
        const button = event.target.closest("#addTransactionButton, #workspaceAddTransaction, #addBudgetButton");
        if (!button) return;
