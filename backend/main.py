...
def create_transaction(data:TransactionCreate,db:Session=Depends(get_db),user=Depends(current_user)):
    item=Transaction(user_id=uid(user),title=data.title,amount=data.amount,type=data.type,category=data.category); db.add(item); db.commit(); db.refresh(item); return item
...
def dashboard(month:str=Depends(validate_month_query),db:Session=Depends(get_db),user=Depends(current_user)):
    u=uid(user);s,e=get_month_range(month);ts=db.query(Transaction).filter(Transaction.user_id==u,Transaction.created_at>=s,Transaction.created_at<e).all();income=sum(t.amount for t in ts if t.type=="income");expense=sum(t.amount for t in ts if t.type=="expense");budgets=db.query(Budget).filter(Budget.user_id==u,Budget.month==month).all();total=sum(b.amount for b in budgets);cats={b.category for b in budgets};spent=sum(t.amount for t in ts if t.type=="expense" and t.category in cats);remaining=total-spent;pct=round(spent/total*100,2) if total else 0;status="no_budget" if not total else "on_track" if remaining>0 else "reached" if remaining==0 else "over_budget";return {"month":month,"income":income,"expense":expense,"balance":income-expense,"total_budget":total,"budget_spent":spent,"budget_remaining":remaining,"budget_spent_percentage":pct,"budget_status":status}
...
def dashboard_budgets(month:str=Depends(validate_month_query),db:Session=Depends(get_db),user=Depends(current_user)):
    ...
    for b in db.query(Budget).filter(Budget.user_id==u,Budget.month==month).all():
        spent=sum(t.amount for t in db.query(Transaction).filter(Transaction.user_id==u,Transaction.type=="expense",Transaction.category==b.category,Transaction.created_at>=s,Transaction.created_at<e).all());rem=b.amount-spent;out.append({"category":b.category,"budget":b.amount,"spent":spent,"remaining":rem,"spent_percentage":round(spent/b.amount*100,2) if b.amount else 0,"status":"on_track" if rem>0 else "reached" if rem==0 else "over_budget"})
...
def budget_summary(month:str=Depends(validate_month_query),db:Session=Depends(get_db),user=Depends(current_user)):
    u=uid(user);s,e=get_month_range(month);out=[]
    for b in db.query(Budget).filter(Budget.user_id==u,Budget.month==month).all():
        spent=sum(t.amount for t in db.query(Transaction).filter(Transaction.user_id==u,Transaction.type=="expense",Transaction.category==b.category,Transaction.created_at>=s,Transaction.created_at<e).all());rem=b.amount-spent;out.append({"category":b.category,"budget":b.amount,"spent":spent,"remaining":rem,"status":"on_track" if rem>0 else "reached" if rem==0 else "over_budget","spent_percentage":round(spent/b.amount*100,2) if b.amount else 0,"remaining_percentage":round(rem/b.amount*100,2) if b.amount else 0})
    return out
