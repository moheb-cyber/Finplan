...
def claim_legacy(user=Depends(current_user)):
    """Attach pre-auth development data to the authenticated workspace."""
    ...
    try:
        ...
        db.query(Budget).filter(Budget.user_id.is_(None)).update(
            {Budget.user_id: user.id}, synchronize_session=False
        )
        db.commit()
